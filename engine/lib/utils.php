...
class utils extends util {
    ...
    static function &array_merge_recursive_distinct(array & $array1, &$array2 = null) {
        ...
        if (is_array($array2)) foreach ($array2 as $key => $val) if (isset($merged[$key]) && is_array($array2[$key])) $merged[$key] = is_array($merged[$key]) ? self::array_merge_recursive_distinct($merged[$key], $array2[$key]) : $array2[$key];
        else $merged[$key] = $val;
    ...
    static function getMWClassName ($mwName) {
    ...
    // Origin: http://php.net/manual/ru/function.parse-ini-string.php#111845
    static function parse_ini_string_m($str) {
