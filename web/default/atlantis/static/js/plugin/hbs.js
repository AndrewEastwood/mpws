...
define('default/js/plugin/hbs', [
    ...
    'default/js/lib/handlebars', 'default/js/lib/underscore', 'default/js/lib/i18nprecompile', 'default/js/lib/json2', 'default/js/lib/handlebarsHelpers'
        ...
        load: function (name, parentRequire, load, config) {
                    ...
                    if (statement && statement.type && statement.type === 'mustache') {
                        ...
                        if (statement.params && typeof Handlebars.helpers[statement.id.string] === 'undefined') {
                            _(statement.params).forEach(function (param) {
                                if (_(paramsWithoutParts).contains(param.original) || param instanceof Handlebars.AST.StringNode || param instanceof Handlebars.AST.IntegerNode || param instanceof Handlebars.AST.BooleanNode || param instanceof Handlebars.AST.SexprNode) {
            ...
            function fetchAndRegister(langMap) {
                fetchText(path, function (text, path) {
                    ...
                    var readCallback = (config.isBuild && config[onHbsReadMethod]) ? config[onHbsReadMethod] : function (name, path, text) {
                        return text
                    };
                    ...
                    for (var i in partials) {
                        if (partials.hasOwnProperty(i) && typeof partials[i] === 'string') { // make sure string, because we're iterating over all props
                            ...
                            deps[i] = "default/js/plugin/hbs!" + path;
                        }
                    }
                    ...
                    helpDepStr = disableHelpers ?
                        '' : (function () {
                            var i;
                            var paths = [];
                            var pathGetter = config.hbs && config.hbs.helperPathCallback ? config.hbs.helperPathCallback : function (name) {
                    ...
                    var options = _.extend(configHbs.compileOptions || {}, {
