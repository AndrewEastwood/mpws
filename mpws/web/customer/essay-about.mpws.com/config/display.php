    $customer['DISPLAY'] = array();
    // customer domain
    $customer['DISPLAY']['DOMAIN'] = 'essay-about.mpws.com';
    // customer home page url
    $customer['DISPLAY']['HOMEPAGE'] = 'http://essay-about.mpws.com/';
    // customer name
    $customer['DISPLAY']['NAME'] = 'Essay-About';
    
    $customer['DISPLAY']['TITLE'] = 'Essay-About';
    
    $customer['DISPLAY']['PUBLIC_MENU'] = Array(
        'SHORT' => Array(
            '/page/about-us.html' => 'About Us',
            '/page/how-it-works.html' => 'How It Works'
        ),
        'RSS' => Array(
        ),
        'MAIN' => Array(
            '/page/buy-essays.html' => 'Buy ready <br />ESSAYS!',
            '/page/free-essays.html' => 'Free <br />ESSAYS!',
            '/page/home.html' => 'Home',
            '/page/faq.html' => 'FAQ',
            '/page/account.html' => 'Join Us',
            '/page/discounts.html' => 'Discounts',
            '/page/prices.html' => 'Prices',
            '/page/terms.html' => 'Terms and Conditions'
        ),
        'FOOTER' => Array(
            '/page/buy-essays.html' => '<strong>Buy ready ESSAYS!</strong>',
            '/page/free-essays.html' => '<strong>FreeESSAYS</strong>',
            '/page/home.html' => 'Home',
            '/page/faq.html' => 'FAQ',
            '/page/account.html' => 'Join Us',
            '/page/discounts.html' => 'Discounts',
            '/page/prices.html' => 'Prices',
            '/page/terms.html' => 'Terms and Conditions'
        ),
        'HEADER' => Array(
            '/page/buy-essays.html' => 'Buy ready ESSAYS!',
            '/page/free-essays.html' => 'FreeESSAYS',
            '/page/home.html' => 'Home',
            '/page/faq.html' => 'FAQ',
            '/page/account.html' => 'Join Us',
            '/page/discounts.html' => 'Discounts',
            '/page/prices.html' => 'Prices',
            '/page/terms.html' => 'Terms and Conditions'
        )
    );
    
    $customer['DISPLAY']['ACCOUNT_MENU'] = array(
        'STUDENT' => Array (
            '/page/account.html?display=orders' => 'Orders', // Active
            //'/page/account.html?display=pending' => 'Pending',
            //'/page/account.html?display=rework' => 'Rework',
            //'/page/account.html?display=completed' => 'Completed',
            '/page/account.html?display=history' => 'History',
            '/page/account.html?display=settings' => 'Settings'
        ),
        'WRITER' => Array (
            '/page/account.html?display=orders' => 'Orders',
            //'/page/account.html?display=pending' => 'Pending',
            //'/page/account.html?display=rework' => 'Rework',
            //'/page/account.html?display=completed' => 'Completed',
            '/page/account.html?display=history' => 'History',
            '/page/account.html?display=settings' => 'Settings'
        )
    );

    $customer['DISPLAY']['KEYWORDS'] = 'test keywords';

    $customer['DISPLAY']['DESCRIPTION'] = 'test desc';
    //
    $customer['DISPLAY']['DESCRIPTION_2'] = 'description test 2';
    //
    $customer['DISPLAY']['PAGE_TITLES'] = array(
        'index' => 'Home',
        'make-order' => 'Make Order',
        'about-us' => 'About Us',
        'user' => 'Account',
        'register' => 'Register',
        'terms' => 'Terms &amp; Conditions',
        'privacy' => 'Privacy Policy',
        'how-it-works' => 'How It Works',
        'contacts' => 'Contact Us',
        'faqs' => 'FAQs'
    );
    //