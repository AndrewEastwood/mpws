...
class libraryDataObject {
    ...
    public function setData($val) {
        if ($val instanceof libraryDataObject)
    ...
    public function process($params = false) {
        ...
        $this->setData($data);
        // $libraryDataObjectObj = new libraryDataObject($data);

        // if (isset($config['output']))
        //     return $libraryDataObjectObj->to($config['output']);
    ...
    public function to($outputType) {
        switch ($outputType) {
            ...
            default:
                throw new Exception("Error Processing Request: libraryDataObject: at to(" . $outputType . "). Wrong format selected", 1);
        }
    }
}
